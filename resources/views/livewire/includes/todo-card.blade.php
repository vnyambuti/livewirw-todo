...
<div class="flex">
    @if ($item->status)
    <input class="mr-2" wire:click="toogleComplete({{$item->id}})" type="checkbox" checked>
    @else
    <input class="mr-2" wire:click="toogleComplete({{$item->id}})" type="checkbox">
    @endif
    @if ($editingId === $item->id)
          <input wire:model="editingTitle" type="text" placeholder="Todo.."
    ...
    @else
    <h3 class="text-lg text-semibold text-gray-800">{{$item->title}}</h3> 
    @endif
        ...
        <div class="flex items-center space-x-2">
            ...
            <button wire:click="delete({{$item->id}})" wire:confirm="Are you sure you want to delete this post?" class="text-sm text-red-500 font-semibold rounded hover:text-teal-800 mr-1">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round"
